...
public class HttpServiceImpl implements HttpService {
  ...
  @NonNull
  public HttpRequest buildGetHttpRequest(
      @NonNull String url, @NonNull LinkedHashMap<String, String> headersOrdered) {
  ...
  private HttpRequest buildHttpRequest(
      ...
      @NonNull LinkedHashMap<String, String> headersOrdered,
      @Nullable BodyPublisher body) {
    final Builder builder = HttpRequest.newBuilder(URI.create(url));

    headersOrdered.forEach(
        (k, v) -> {
          if (Objects.nonNull(k) && Objects.nonNull(v)) builder.header(k, v);
        });
