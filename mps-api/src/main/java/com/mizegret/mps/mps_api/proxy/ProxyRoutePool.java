...
public class ProxyRoutePool{
    ...
    public ProxyRoutePool() throws IOException {
        ...
        try (final BufferedReader br = new BufferedReader(
                new InputStreamReader(getClass().getResourceAsStream("/proxy-list.txt"), StandardCharsets.UTF_8))){
            proxies = br.lines().map(String::trim).map(line -> line.split(":", 4))
                    .map(l -> new Proxy(l[0], Integer.parseInt(l[1]), l[2], l[3])).toList();
        }
        this.proxyRoutes = proxies.stream()
                .map(p -> new ProxyRoute(HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(10))
